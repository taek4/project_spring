...
<div class="container-fluid px-4">
	<div class="button-container">
		<h3 class="mt-4">販売入力</h3>
		<button id="searchButton" class="button">販売入力</button>
	</div>
	<form id="saleForm" method="post" action="<c:url value='/sale/save'/>">
	<fieldset>
		<div>
			<ul>
				<li>
					...
					<input type="date" id="hitsuke" name="hitsuke">
				</li>
				<li>
				...
				<li>
					<div id="add-filter">
		    			<label for="add">新項目追加</label>	    				
		    			<input type="text" id="addGroupText" name="addGroup" placeholder="新項目追加">
					</div>
				</li>
				<li>
					<span>添付</span>
			...
			<button class="button">主文</button>
			<button class="button">購買</button>
			<button class="button">保留</button>
			<button class="button">割引</button>
			<button class="button">在庫ロード</button>
			<button class="button">バーコード</button>
			<button class="button">伝票バーコード</button>
			<button class="button">検証</button>
			<button class="button">利益計算</button>
			<button class="button">伝票ロード</button>
			<table id="itemTable">
				<thead>
					<tr>
						<th>品目選択</th>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function calculateKyoukyuuGaku(row) {
        ...
        row.querySelector('.kyoukyuuGaku').value = kyoukyuuGaku.toFixed(2); // 소수점 2자리로 고정 // 小数点2桁に固定
    }
    ...
    // 테이블 내 삭제 버튼 클릭 이벤트 // テーブル内の削除ボタンクリックイベント
        ...
        rows.forEach((row, index) => {
            row.querySelectorAll('input, select').forEach(input => {
                if (input.name) {
                    ...
                    // 문자열 연결 방식으로 name 속성 업데이트 // 文字列結合方式でname属性を更新
    ...
    // 기존 행 및 새로 추가된 행에 이벤트 바인딩 // 既存行および新しく追加された行にイベントをバインド
    function bindRowEvents(row) {
        ...
        row.querySelector('.hinmokuSelect').addEventListener('change', function () {
            ...
            calculateKyoukyuuGaku(row); // 공급가액 계산 // 供給価額計算
        });
    ...
    document.getElementById('saleForm').addEventListener('submit', function (event) {
        ...
        if (!isValid) {
            event.preventDefault(); // 제출 중단 // 提出を中断
        }
    });
});
</script>
